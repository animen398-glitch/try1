...
| Метрика | Значение |
|---|---|
| Тесты | **269 passed, 1 skipped** (сетенезависимые, Qt headless) |
| Линтер (ruff) | ✅ чисто |
| Компиляция всех модулей | ✅ 0 ошибок |
| `except:` без типа | 0 |
| Маркеры TODO/FIXME/XXX | 0 |
| Своих модулей / тест-файлов | 69 / 46 |
...
| Тема | Статус / заметка |
|---|---|
| Папка `~/` на ~51 ГБ | Легаси-данные сканов (gitignored). Текущий код пишет в правильный home — **папку можно удалить, освободив ~51 ГБ** (см. §6). |
...
**Гигиена (быстро):**
1. **Запушить 4 локальных коммита** на `origin/master`: Dashboard «Очистить»,
   детали строки + копирование, Settings «Очистить кеш», tilde-fix.
2. **Удалить `~/` (~51 ГБ)** — освободить место (данные легаси, не нужны коду).
