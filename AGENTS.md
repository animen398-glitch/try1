...
**Последние важные изменения на 2026-07-04 (GUI: пагинация вкладки Remediation):**
- Гигиена репо: удалены три untracked-каталога `.pytest-*-rollback-*` (мусор от Windows pytest-teardown) + добавлены ignore-паттерны в `.gitignore` (commit `e9a64bd7`). Затем — пагинация вкладки **Remediation** (`gui/tab_remediation.py`): список задач ремедиации масштабируется как Findings (задача-на-находку по проекту), но грузился весь разом. Применён готовый `TablePaginator` ТОЧНО по образцу Findings/Assets/Timeline: рендер строки вынесен в static `_render_rem_row`, `_populate_rem_table` → `paginator.set_rows` (полный `_rem_records` хранится для selection), `_selected_rem` маппит строку через `paginator.record_at`, `_on_rem_page_changed` чистит stale detail/edit. Это уточняет прежнюю классификацию (Remediation ошибочно числился «bounded-small»); прочие оставшиеся таблицы (security/cookie — per-page audit; attack-paths — capped; overview/osint — bounded; subdomain — streaming) остаются непагинированными осознанно. Тесты: `test_remediation_tab.py` (+1: 250 задач → окно 200, выбор строки на 2-й странице → верный full-index; набор 2905 → 2906). Проверено: ruff clean, полный offline pytest exit 0.

**Последние важные изменения на 2026-07-03 (Windows CLI Process Hardening):**
- Проход надёжности запуска внешних CLI на Windows: ни одного всплывающего cmd/conhost/PowerShell-окна из GUI, единый отменяемый процессный контракт, архитектурный guard и redacted-журнал. Всё АДДИТИВНО; бизнес-логика сканеров и форматы находок не тронуты; `build.spec` `console=False` сохранён. Аудит: prod-код уже шёл через `run_hidden`/`popen_hidden`/`run_command`; `os.system`/`shell=True`/`QProcess`/`startDetached`/`check_*` в prod отсутствуют (`os.startfile` для открытия каталога — не запуск CLI, оставлен).
- **AST-guard** (`c1d1603b`): `tests/test_no_direct_subprocess.py` парсит AST каждого prod-файла (core/gui/utils/remote/plugins + root-скрипты; tests/vendor/tools/build исключены) и запрещает прямой spawn (`subprocess.run/Popen/call/check_call/check_output`, `os.system`, `shell=True`) вне единственного шва `utils/subprocess_utils.py`; следует from-import алиасам.
- **Отменяемый контракт** (`c1d1603b`): `utils/subprocess_utils.run_capture` (Popen + poll-loop: timeout + отмена + kill дерева процессов), `terminate_process_tree` (`taskkill /F /T` на Windows, `killpg` на POSIX), `CancellationToken`. Результат `{rc,stdout,stderr,timed_out,cancelled,truncated,error}`, never-raise, cap на объём. `external_tools.run_command` += аддитивный `cancel_event` (дефолтный путь `run_hidden`/`subprocess.run` байт-совместим — существующие монкипатчи целы).
- **Журнал + redaction** (`c1d1603b`): `system_logger.journal_tool_run` — одна redacted-строка на прогон (tool/status/rc/duration + хвост stderr) в тот же `system.log`, что показывает System-таб (не вторая система логирования); `redact_command` скрывает токены/секреты/Authorization/Cookie/чувствительные аргументы; публичный `crash_reporter.redact()` — переиспользуемый шов. Адаптеры (nuclei/katana/amass/subfinder/httpx/bbot/lift/scrapy) получили `set_cancel_event`; Scrapy-таб — кнопку «Отменить» через общий `CancellationToken`.
- **Промптная отмена** (`dff008c`): кнопки «Остановить»/«Stop» Full Collection и subdomain-скана уже существовали (кооперативная отмена движка между фазами); теперь движки пробрасывают свой cancel-Event в внешние runner'ы (`collection_runner`: nuclei/katana/bbot/lift; `subdomain_scanner`: amass/subfinder/httpx) → прогон идёт по `run_capture`, и Stop сразу убивает дерево дочерних процессов вместо ожидания их таймаута.
- Проверено: ruff clean; полный offline pytest **exit 0** (+39 тестов, набор **2866 → 2905**); self-check 29 вкладок; PyInstaller (QT_API=pyside6, `console=False`) + frozen self-check 29 вкладок; безопасный version-smoke реальных бинарников через скрытый seam — nuclei v3.9.0/katana/amass v5.1.1/subfinder v2.14.0/httpx/bbot v2.8.4 (rc=0, ни одного консольного окна; rar/winrar SKIPPED). Локальные коммиты (`c1d1603b`, `dff008c`), без push. Детальный лог — `PROJECT_STATUS.txt`.

**Последние важные изменения на 2026-07-03 (Authorized Enterprise Recon E1–E10 + increment-2 depth):**
- Отдельный трек «Authorized Enterprise Recon & Client-Safe Scaling» (детальный лог — `DOCS_DEVELOPMENT_PLAYBOOK.md`, единственный SoT его прогресса; PROJECT_STATUS.txt его не дублирует). Все 10 эпиков E1–E10 приземлены ранее (passive OSINT Shodan/Censys/InternetDB — zero target traffic; E2 Coverage Gate; execution profiles; browser-accuracy; context-aware wordlist planner; origin-exposure intel; storage-backend seam; worker orchestration; sensitive-data governance; parser hardening). Все гардрейлы authorized-only соблюдены (нет stealth/evasion/proxy-rotation/TOR/JA3-spoof/brute-force/exploit/persistence).
- **Increment-2 depth (этот проход, 4 коммита, каждый — зелёный полный pytest):**
  - **E3-2** (Stage 15, `0e193336`): scan-time execution-profile IP gate. Чистый `execution_profile.enforce_target_ip(ip, profile, *, now, enabled)` (expiry-first + `ip_authorized`, строго opt-in). `CollectionRunner` pre-flight `_enforce_execution_profile(url)` (инъектируемый `_ip_resolver`; резолв цели один раз до создания workspace; **fail-closed** — нерезолвящийся хост отклоняется) + `_execution_gate()` (instance-override / settings `execution_enforcement`). `ExecutionNotAuthorized`. Off by default → прогоны байт-идентичны.
  - **E4-2** (Stage 16, `bac7fd27`): browser-backed accuracy collection phase. `_phase_browser_accuracy` (рендер через `browser_accuracy.capture_rendered_html` + статик-фетч по общему HTTP-seam → `build_browser_accuracy` delta), `guarded`+`_PHASE_ORDER`, Skip→E2 coverage `missing_dependency` без Playwright; report-card + monitor/GUI parity. Инъектируемые seam'ы `_accuracy_render_fn`/`_accuracy_static_fetch`.
  - **E5-2** (Stage 17, `e680924a`): context-aware path-probe phase. `core/path_prober.py` (чистый executor + injectable fetch seam; `classify_status`/`probe_paths`). `_phase_path_probe` исполняет `wordlist_manager.plan_wordlist` (Skip на passive-only ROE), `_default_probe_fetch` — throttled HEAD; `guarded` deps=recon; report-card + parity. Дважды гейтед (Scope Guard + planner), бюджет ROE ограничивает кол-во (не брутфорс). Findings-fold обнаруженных путей отложен.
  - **E7-2** (Stage 18, `669274d2`): dialect ops + реальный `PostgresBackend`. `DatabaseBackend`+`SQLiteBackend` получили `get/set_schema_version`/`table_columns`/`build_upsert` (SQLite — байт-идентичный старому inline SQL); `PostgresBackend` (`%s`, `ON CONFLICT`, `information_schema`, metadata-таблица версии, lazy psycopg/psycopg2 + `_DBAPIConnectionAdapter`); `resolve_backend('postgres://')` → его. `SQLiteStore` роутит generic-ops через backend (`_columns`/`_insert_rows` → instance-методы; `_add_column` остался static SQLite-хелпером). Полный multi-store Postgres (per-store DDL/SQL + conflict-ключи) — отдельный задокументированный трек.
- Тесты: `test_path_prober.py` (8), `test_db_backend.py` (+9, 1 переписан), `test_execution_profile.py` (+5), `test_collection_runner.py` (+15). Проверено на каждом коммите: ruff clean, self-check 29 вкладок, полный offline pytest зелёный (exit 0). **Follow-up E5-2 (commit `4291a5df`):** fold обнаруженных *читаемых* чувствительных путей в находки — `path_prober.exposure_findings` (курируемая карта `_SENSITIVE_PATHS`; recon-поверхность и auth-gated `protected` не промоутятся) + `_path_probe_findings` фолдится в vuln-находки в `_run_active_phases` (паттерн `_takeover_findings`; category `exposed_path`); +10 тестов. **Follow-up E4-2 (commit `c4be70a8`):** web-console parity дельты browser-accuracy — `web_app._browser_accuracy_view` + `GET /browser-accuracy` (тонкий report-based read; отдельный маршрут от `/accuracy` = detection-confidence по сущностям) + dashboard-кнопка «Browser Accuracy» (`showBrowserAccuracy()`, mirror `showAccuracy`); +5 тестов. Рост набора: **2603 → 2866**. Локальные коммиты, без push. Все E1–E10 (контракты + increment-2) закрыты.
