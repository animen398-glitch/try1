...
- `core/mission_store.py` — Mission Center M2: `MissionStore(SQLiteStore)` (single-table `missions`, без events; save_mission валидирует через `mission_to_json`+схему; get/list/delete/export). Участвует в `project_io` bundle (`missions.json`) через events-less `PROJECT_EXPORT`. GUI/web/timeline — M3+.

**Последние важные изменения на 2026-06-28 (Mission Center M2 — persistence + bundle):**
...
**Тестовый ориентир:**
- `PROJECT_REPORT.md` указывает актуальный масштаб набора; на 2026-06-28 (после Mission Center M2) — 2177 offline/headless тестов (зелёные, 1 Starlette/httpx warning).
