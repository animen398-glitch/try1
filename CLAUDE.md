...
**Запрещено** выполнять любые действия с удалёнными репозиториями и публикацией:
`git push`, `git commit`, `git fetch`, `git pull`, `git clone`, создание PR,
изменение `remote`/`origin`, релизы, любые действия с GitHub.

Разрешена **только локальная** работа с файлами проекта (чтение, правка,
создание, запуск тестов локально). Версионирование коммитит человек сам.
