...
- `core/tool_parsers.py` — Per-Tool Offline Parsers (слой над M3): `parse_tool_output(tool, evidence)` превращает уже-захваченный evidence в generic `{findings, assets}` для `map_tool_result_to_findings`; БЕЗ запуска tools/сети/store-writes. Парсеры header/cookie/source-map переиспользуют `core.audit_checks` (без дублирования), `safe_active_prober` — asset/enumeration. Реестр `PARSERS` (additive); unknown/empty/unparsed tool → `ValueError`.
...
**Последние важные изменения на 2026-06-30 (Offline Tool-Evidence Pipeline):**
...
**Тестовый ориентир:**
- `PROJECT_REPORT.md` указывает актуальный масштаб набора; на 2026-06-30 (после Offline Tool-Evidence Pipeline) — 2287 offline/headless тестов (зелёные, 1 Starlette/httpx warning).
