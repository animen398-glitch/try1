...
tests/                  # pytest-набор (~2865 тестов, offline/headless)
...
- Тесты: `test_path_prober.py` (8), `test_db_backend.py` (+9, 1 переписан), `test_execution_profile.py` (+5), `test_collection_runner.py` (+15). Проверено на каждом коммите: ruff clean, self-check 29 вкладок, полный offline pytest зелёный (exit 0). **Follow-up E5-2 (commit `4291a5df`):** fold обнаруженных *читаемых* чувствительных путей в находки — `path_prober.exposure_findings` (курируемая карта `_SENSITIVE_PATHS`; recon-поверхность и auth-gated `protected` не промоутятся) + `_path_probe_findings` фолдится в vuln-находки в `_run_active_phases` (паттерн `_takeover_findings`; category `exposed_path`); +10 тестов. **Follow-up E4-2 (commit `c4be70a8`):** web-console parity дельты browser-accuracy — `web_app._browser_accuracy_view` + `GET /browser-accuracy` (тонкий report-based read; отдельный маршрут от `/accuracy` = detection-confidence по сущностям); +4 теста. Рост набора: **2603 → 2865**. Локальные коммиты, без push. Все E1–E10 (контракты + increment-2) закрыты.
...
**Тестовый ориентир:**
- `PROJECT_REPORT.md` указывает актуальный масштаб набора; на 2026-07-03 (после Authorized Enterprise Recon E1–E10 + increment-2 depth E3-2/E4-2/E5-2/E7-2 + E5-2 findings-fold + E4-2 web parity) — 2865 offline/headless теста (зелёные, 1 Starlette/httpx warning); GUI — 29 вкладок.
